...
export function create_peer_connection(lv, from_user, offer) {
  ...
  let new_peer_connection = new RTCPeerConnection({
    iceServers: [
  ...
  if (offer !== undefined) {
    ...
    new_peer_connection
      ...
      .then((answer) => {
        ...
        lv.pushEvent("new_answer", { toUser: from_user, description: answer });
      })
      .catch((err) => console.log(err));
  }

  new_peer_connection.onicecandidate = async ({ candidate }) => {
    // from_user is the new value for toUser because we're sending this data back
    // to the sender
    lv.pushEvent("new_ice_candidate", { toUser: from_user, candidate });
  };
  ...
  new_peer_connection.ontrack = async (event) => {
    ...
    document.getElementById(`video-remote-${from_user}`).srcObject =
      event.streams[0];
  };
